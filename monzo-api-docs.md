...
Examples in this documentation are written using [httpie](https://httpie.io/) for clarity. To install httpie on macOS run `brew install httpie`.
...
| `since` (Optional) | An RFC 3339-encoded timestamp (e.g. `2009-11-10T23:00:00Z`) or an object id (e.g. `tx_00008zhJ3kE6c8kmsGUKgn`) |
| `before` (Optional) | An RFC 3339 encoded-timestamp (e.g. `2009-11-10T23:00:00Z`) |
...
| `total_balance` | The sum of the currently available balance of the account and the combined total of all pots. |
| `currency` | The ISO 4217 currency code. |
| `spend_today` | The amount spent today (considered from approx 4am onwards), as a negative 64bit integer in minor units of the currency. |
...
| `settled` | The timestamp at which the transaction settled. If this field is an empty string, the transaction is authorised but not yet "complete." |
| `category` | The category can be set for each transaction by the user. Over time we learn which merchant goes in which category and auto-assign the category of a transaction. If the user hasn't set a category, we'll return the default category of the merchant on this transaction. Top-ups have category set to `mondo`. Valid values are `general`, `eating_out`, `expenses`, `transport`, `cash`, `bills`, `entertainment`, `shopping`, `holidays`, `groceries`. |
...
| Parameter | Description |
|-----------|-------------|
| `expand[]` (Repeated) | This argument can repeat. If `merchant` is present, the merchant will be expanded inline (rather than a merchant id being provided). |
...
| `since` (Optional) | Start time as RFC3339 encoded timestamp (e.g. `2009-11-10T23:00:00Z`) |
| `before` (Optional) | End time as RFC3339 encoded timestamp (e.g. `2009-11-10T23:00:00Z`) |
...
| `background_color` (Optional) | Hex value for the background colour of the feed item in the format `#RRGGBB`. Defaults to standard app colours (ie. white background). |
...
- [Register](#register-attachment) the attachment against a transaction.
...
| `taxes` | Taxes (e.g. VAT) added onto the total. |
| `payments` | Payments, indicating how the customer paid the total. |
| `merchant` | Merchant you shopped at. (This is a different type of object than the Merchant on a Transaction.) |
